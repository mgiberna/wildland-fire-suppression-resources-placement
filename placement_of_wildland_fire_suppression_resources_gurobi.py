...
def fire_resource_placement_model(dict, setup1):
    G = dict["graph"]
...
def optimize_and_write_model(dataset, setup):
    ...
    for dict in dataset:
        print("Creating and Optimizing Model for Instance #" + dict["instance"] + "...")
        t_0 = time.time()
        FRP = fire_resource_placement_model(dict, setup1=setup)
        ...
        if FRP.status == gb.GRB.OPTIMAL:
            ...
            names_to_retrieve = (f"Y[{i},{h}]" for i in dataset[dict["instance"] - 1]["graph"].nodes())
        ...
        if dict["instance"] == 1:
            with open("dataset_optimised_5resources.csv", "w", newline="") as fp:
                writer = csv.DictWriter(fp, fieldnames=dataset[dict["instance"] - 1].keys())
                writer.writeheader()
                writer.writerow(dataset[dict["instance"] - 1])

        with open("dataset_optimised_5resources.csv", "a", newline="") as fp:
            writer = csv.DictWriter(fp, fieldnames=dataset[dict["instance"] - 1].keys())
            writer.writerow(dataset[dict["instance"] - 1])

    print(dataset)
    with open('dataset_optimised_5resources.pickle', 'wb') as file:
